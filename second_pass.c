...
void second_pass (char *fileName, symbol * symTable,object * objects) {
    ...
    char *token,*tokenB;
    char delimit[] = " \n\t\r ,";
    char cleanLine[MAX_BINARY] = "00000000000000000000\0",binaryLine[MAX_BINARY];
...
int* middleWordProcess(char *firstOperand, char *secondOperand, int funct, symbol *symTable, int symTableSize, FILE *newCode,extAddress *extAdd,int extSize, int printCounter,int errorFlag) {
    ...
    char cleanLine[MAX_BINARY] = "00000000000000000000\0",line[MAX_BINARY];
...
void bitTranslation(int bitLength, int index, int value, char line[]) {
    ...
    index = index-bitLength+1; /*we want the reminder result to be enter the opposite way*/

    if (value < 0) { /*negative number requires a calculation with the "mashlim le2" method*/
        negative = TRUE;
