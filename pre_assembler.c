...
void pre_assembler(FILE *sFile, char *fileName) {
    ...
    int macroFlag = FALSE, firstLine = TRUE, endmFlag=FALSE;

    string macroStr = "macro", endmStr = "endm",line,copyLine;
    ...
    if (!macTable) {
        ...
        while (fgets(line, MAX_CHAR, sFile) != NULL) {
            ...
            if (strstr(line, endmStr)) /* endm identified */
                endmFlag=TRUE;
            else { /*only relevant when no endm found*/
